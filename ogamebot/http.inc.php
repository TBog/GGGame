				...
				if ( $this->_response->get_header( 'Content-Length' ) != null ) {
					...
					if ( $this->_response->get_header( 'Transfer-Encoding' ) != null ) {
						if ( strtolower( $this->_response->get_header( 'Transfer-Encoding' ) ) == 'chunked' ) {
							...
							$tmp = trim(fgets($this->_socket, 4096));
							$chunk_size = (integer)hexdec( $tmp );
							while($chunk_size > 0) {
								// if ($chunk_size >= 8192 )
									// die(file_put_contents('debug.txt', "chunk_size = $chunk_size | tmp = $tmp | $body") . "error");
								$body .= fullread( $this->_socket, $chunk_size );
								$tmp = fullread( $this->_socket, strlen(HTTP_CRLF) );
								if (trim($tmp) != '')
									die(file_put_contents('debug.txt', "what is this ? '$tmp' ?") . "error");
								$tmp = trim(fgets( $this->_socket, 4096));
								$chunk_size = (integer)hexdec( $tmp );
							}
